...
public class AddrSpace {
	...
	public AddrSpace(OpenFile executable) {
		...
		Arrays.fill(Machine.mMainMemory, 0, size, (byte) 0);

		// Copy the code segment into memory
		if (noffH.code.size > 0) {
			Debug.print('a', "Initializing code segment, at " + noffH.code.virtualAddr + ", size " + noffH.code.size);

			// Create a temporary buffer to copy the code
			byte[] codeBytes = new byte[noffH.code.size];

			// read the code into the buffer
			executable.readAt(codeBytes, noffH.code.size, noffH.code.inFileAddr);

			// Copy the buffer into the main memory
			System.arraycopy(codeBytes, 0, Machine.mMainMemory, noffH.code.virtualAddr, noffH.code.size);
		}

		// Copy the data segments into memory
		if (noffH.initData.size > 0) {
			Debug.print('a',
					"Initializing data segment, at " + noffH.initData.virtualAddr + ", size " + noffH.initData.size);

			// Create a temporary buffer for the data sections
			byte[] initDataBytes = new byte[noffH.initData.size];

			// Read in the data sections
			executable.readAt(initDataBytes, noffH.initData.size, noffH.initData.inFileAddr);

			// Copy the buffer into the main memory
			System.arraycopy((Object) initDataBytes, 0, (Object) Machine.mMainMemory, noffH.initData.virtualAddr,
					noffH.initData.size);
		}
	}
